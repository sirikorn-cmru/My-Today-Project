...
| Get Related Files for Event | `eventId` | รายการ File ที่ `linkedEventIds` มี `eventId` นี้อยู่ | Sprint 10 — นี่คือ query ไม่ใช่ field ที่เก็บบน Event (ดูหัวข้อ 3), กลไกเดียวกับ "Get Related Files for Task" |
...
Operation กลุ่มนี้ narrative ของ Sprint ที่ยังไม่เริ่ม (ตาม backlog.md ณ วันที่ 20260825 — Sprint 1-10 ทั้งหมด verified เสร็จแล้ว, Sprint 11 "กำลังดำเนินการ" โดย Get Storage Usage Estimate/Warn Near Quota เป็นชิ้นแรกที่ build เสร็จแล้ว ส่วนที่เหลือของ Sprint 11 ยังไม่เสร็จ) บอกเป็นนัยว่าจะต้องมี แต่ **ยังไม่ถูกสร้างจริงใน Domain Logic Layer ปัจจุบัน**:

(ไม่มี known gap คงเหลือในหัวข้อนี้ ณ ตอนนี้ — ดู Change Log หัวข้อ 5 สำหรับการย้าย "Get Storage Usage Estimate / Warn Near Quota" ไปหัวข้อ 3 เมื่อ 20260825)
