...
รายการนี้คือ container/flow ที่ narrative ของ Sprint 11 (Sprint สุดท้ายของ Competition Track ที่เหลือ) บอกเป็นนัยว่าจะต้องมี ตามสถานะล่าสุดใน backlog.md (ตรวจสอบล่าสุด 20260825 — Sprint 11 เปลี่ยนสถานะเป็น "กำลังดำเนินการ" แล้ว ไม่ใช่ "ยังไม่เริ่ม" เหมือนที่บันทึกไว้ก่อนหน้า: มีความคืบหน้าจริงบางส่วนใน `src/` แม้ยังไม่มี commit เข้า git — IndexedDB Quota-Warning ถูก build และตรวจสอบใน browser แล้วเสร็จสมบูรณ์ จึงตัดออกจากรายการ known gap ด้านล่าง ส่วน Accessibility Baseline ถูก build เพียงบางส่วนเท่านั้น จึงยังคงค้างอยู่ในรายการนี้แต่ปรับถ้อยคำให้สะท้อนความคืบหน้าที่มีจริง — **ยังไม่ใช่การประกาศว่า Sprint 11 เสร็จสมบูรณ์**):
...
Sprint 11 (Demo/Polish/Freeze) ไม่เพิ่ม container ใหม่ แต่เพิ่มพฤติกรรมใหม่ให้กับ container ที่มีอยู่แล้ว — Accessibility Baseline (บางส่วน) กระทบ Presentation/Interaction Layer เท่านั้นที่ยังเหลือเป็น known gap ณ ตอนนี้ ส่วน IndexedDB Quota-Warning ที่เคยกระทบ Binary/Blob Local Persistence ถูก build เสร็จสมบูรณ์และตัดออกจากหัวข้อนี้แล้ว (ดู Change Log 20260825) ส่วน Browser Compatibility Matrix (อีกหนึ่งรายการ NFR ที่ Sprint 11 รับผิดชอบ) ไม่นับเป็นรายการในหัวข้อนี้ เพราะเป็นขอบเขตการทดสอบ ไม่ใช่ gap เชิงสถาปัตยกรรม
