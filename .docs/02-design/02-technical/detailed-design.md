...
### 2.7 Enable Cloud Sync (Sprint 12, opt-in)
...
sequenceDiagram
    ...
    participant Presentation as Presentation / Interaction Layer
    participant Domain as Application / Domain Logic Layer
    participant CloudSync as Cloud Synchronization Service
    participant StructPersist as Structured Local Persistence
    participant IdentityProvider as External Identity Provider
    participant CloudStore as Cloud Data Store

    User->>Presentation: กด "Sign in ด้วย Google" ที่หน้า Profile
    Presentation->>Domain: Sign In()
    Domain->>CloudSync: Sign In()
    CloudSync->>IdentityProvider: ยืนยันตัวตน (external auth flow)
    IdentityProvider-->>CloudSync: ตัวตนที่ยืนยันแล้ว
    CloudSync-->>Domain: signed in สำเร็จ
    Domain-->>Presentation: signed in สำเร็จ

    User->>Presentation: เปิด toggle "เปิดใช้ Cloud Sync" (ต้อง sign in มาก่อนแล้วเท่านั้นถึงจะกดได้)
    Presentation->>Domain: Enable Cloud Sync()
    Domain->>CloudSync: Enable Cloud Sync()
    CloudSync->>StructPersist: อ่าน record ปัจจุบันของ Life Area/Task/Event/Note/Link/Profile (ไม่รวม File)
    StructPersist-->>CloudSync: record ฝั่ง local
    CloudSync->>CloudStore: Pull and Merge from Cloud (อ่าน record ฝั่ง cloud มาเทียบ)
    CloudStore-->>CloudSync: record ฝั่ง cloud (ถ้ามี)
    CloudSync->>CloudSync: เทียบ updatedAt เลือก record ที่ใหม่กว่าต่อ id (last-write-wins)
    CloudSync->>StructPersist: เขียน record ที่ merge แล้วกลับลง local
    StructPersist-->>CloudSync: ยืนยันเขียนสำเร็จ
    CloudSync-->>Domain: sync เปิดใช้งานสำเร็จ ข้อมูล merge แล้ว
    Domain-->>Presentation: sync เปิดใช้งานสำเร็จ ข้อมูล merge แล้ว
    Presentation-->>User: แสดงสถานะ sync = เปิด, merge เสร็จ
```

**Push Diff to Cloud (ไม่มีไดอะแกรมแยก — อธิบายแบบร้อยแก้วต่อท้าย ตาม pattern เดียวกับ 2.6):** หลังจากเปิด sync สำเร็จตามไดอะแกรมข้างต้นแล้ว **Push Diff to Cloud** ทำงานเป็น background operation แบบ debounce ทุกครั้งที่ record ฝั่ง local ของ 6 entity ที่ sync ได้ (Life Area/Task/Event/Note/Link/Profile) เปลี่ยนแปลง ขณะ sync เปิดอยู่เท่านั้น — Application/Domain Logic Layer อ่าน record ปัจจุบันจาก Structured Local Persistence แล้วส่งต่อให้ Cloud Synchronization Service เทียบ `updatedAt` กับ record บน Cloud Data Store ต่อ id (upsert record ถ้าฝั่ง local ใหม่กว่า, ลบ record ฝั่ง cloud ถ้า id นั้นถูกลบไปแล้วฝั่ง local) — **ไม่เคย block การเขียนลง Structured Local Persistence ที่เกิดขึ้นก่อนหน้าเลย** (Sprint 12 Business Rule ข้อ 1 — local-first เสมอ การเขียน local เสร็จสมบูรณ์ไปแล้วก่อนที่ Push Diff to Cloud จะเริ่มทำงานด้วยซ้ำ)

อ้างอิง: [[api-spec|api-spec.md]] หัวข้อ 3 (Cloud Synchronization Operations) แถว "Sign In"/"Enable Cloud Sync"/"Pull and Merge from Cloud"/"Push Diff to Cloud" — ระบุชัดว่านี่คือ**ไดอะแกรมแรกในเอกสารทั้งฉบับที่มี message ข้ามเครือข่ายจริง** (ไปยัง `IdentityProvider`/`CloudStore` ภายนอก) ต่างจากทุกไดอะแกรมอื่นในเอกสารนี้ (§1, §2.1-2.6) ที่เป็น local call ล้วนๆ ทั้งหมด — participant `CloudSync`/`IdentityProvider`/`CloudStore` ในไดอะแกรมนี้ตรงกับ container "Cloud Synchronization Service" และ external system "External Identity Provider"/"Cloud Data Store" ที่ [[architecture|architecture.md]] หัวข้อ 1/2 นิยามไว้แล้วเท่านั้น ไม่มีการเพิ่ม container ใหม่
...
- **Enable Cloud Sync (2.7, Sprint 12):** ถ้า Sign In ล้มเหลว (เช่น External Identity Provider ยังไม่เปิดใช้ provider ที่เลือก, popup ถูกบล็อก) ระบบแสดง error แบบ dismissible ที่ Presentation / Interaction Layer โดยไม่กระทบการทำงานอื่นใดของแอปเลย (Sprint 12 Business Rule ข้อ 9 — non-regression) — ถ้า Pull and Merge from Cloud หรือ Push Diff to Cloud ล้มเหลว (เช่นไม่มีเครือข่าย) ข้อมูลฝั่ง Structured Local Persistence ไม่ถูกแตะต้อง/ไม่สูญหายเลย มีเพียงสถานะ sync ที่แสดงผิดพลาด แล้วผู้ใช้ยังใช้งานส่วนอื่นของแอปต่อได้ตามปกติทันที (ตรงตาม Acceptance Criteria ของ Sprint 12 ที่ระบุว่าปิดเครือข่ายแล้วแอปยังใช้งานได้ปกติ — local ยังเป็น source of truth หลักเสมอ)

## 4. Known Gaps / Not-yet-built Flows

รายการนี้สอดคล้องกับ known gaps ใน [[architecture|architecture.md]] หัวข้อ 6 และ [[api-spec|api-spec.md]] หัวข้อ 5 — ตรวจสอบล่าสุด 20260830: ตาม backlog.md **Sprint 12 (Cloud Sync) build เสร็จสมบูรณ์แล้ว** จึงได้ diagram จริงในหัวข้อ 2.7 ข้างบนแล้ว ไม่ใช่ known gap อีกต่อไป — ส่วน **Sprint 13 (Smart Capture จากรูปภาพ) มีแค่ spec ยังไม่มีโค้ดจริงสักบรรทัดเดียว** จึงคงเหลือเป็น known gap เดียวในหัวข้อนี้:

- **Analyze Image for Event Capture** (Sprint 13, ยังไม่ build — มีแค่ spec) — flow ที่วางแผนไว้: ผู้ใช้เลือกรูปภาพที่หน้า Quick Capture ประเภท Event (ต้อง Sign In ก่อนเสมอ — เกทเดียวกับที่ §2.7 ใช้อยู่แล้ว) → Presentation / Interaction Layer ส่งรูปไปยัง Application / Domain Logic Layer → Domain Logic Layer ต้องส่งต่อผ่าน **Server-side Integration Proxy** (container ใหม่ที่ยังไม่มีอยู่จริง — [[architecture|architecture.md]] หัวข้อ 6 ระบุเป็น known gap เดียวกันนี้) ก่อนจะถึง **External AI Analysis Service** ภายนอก → ผลลัพธ์ที่สกัดได้ (title/date/time/location) ส่งกลับมาเติมฟอร์ม Quick Capture ประเภท Event ให้ผู้ใช้ตรวจสอบ/แก้ไขก่อนบันทึกเสมอ (ห้าม auto-submit) — ไม่วาด sequence diagram ให้ ตามกฎของเอกสารนี้ที่วาดได้เฉพาะ flow ที่ build จริงแล้วเท่านั้น — **นี่จะเป็นไดอะแกรมที่ 2 ในเอกสารทั้งฉบับที่มี message ข้ามเครือข่ายจริง เมื่อ build จริง** แยกจาก §2.7 ของ Sprint 12 โดยสิ้นเชิง (คนละ container คนละเหตุผล: §2.7 คุยกับ External Identity Provider/Cloud Data Store เพื่อ sync ข้อมูลโครงสร้าง ส่วน Sprint 13 คุยผ่าน Server-side Integration Proxy เพื่อซ่อน secret ของ AI provider ไม่ให้หลุดไปฝั่ง client) อ้างอิง: [[api-spec|api-spec.md]] หัวข้อ 5 Known Gaps แถว "Analyze Image for Event Capture"

ส่วนที่เหลือของ Sprint 11 ที่ backlog.md ระบุว่ายังไม่เสร็จ (Browser Compatibility Matrix/NFR-06, การตรวจสอบ Accessibility Baseline/NFR-04 อย่างเป็นทางการส่วน keyboard-focus/contrast, Competition Demo script, Black Box Testing เป็นทางการ, Gate 11) ไม่ใช่ flow หรือ operation ใหม่ระดับ persona journey/cross-cutting operation ที่ต้องมี sequence diagram ของตัวเอง — เป็นงานด้าน testing/verification/polish ที่ไม่กระทบรูปร่างของ message flow ใดๆ ที่บันทึกไว้ในเอกสารนี้ จึงไม่มีบรรทัดเพิ่มในหัวข้อ Known Gaps นี้สำหรับรายการเหล่านั้น — เช่นเดียวกัน Gate 12 ของ Sprint 12 ที่ยังไม่ผ่านครบ 100% (Security Rules Emulator test ต้องการ Java 21+, การยืนยัน multi-device sync แบบเต็มรูปแบบในเบราว์เซอร์จริง) ก็เป็นงาน testing/verification เท่านั้น ไม่กระทบรูปร่างของ message flow ที่ §2.7 บันทึกไว้ (โค้ด build จริงและ non-regression ยืนยันแล้วในเบราว์เซอร์)
...
### 2.7 Enable Cloud Sync (Sprint 12)

Presentation = ส่วน Cloud Sync card ใน `src/pages/ProfilePage.tsx` (ปุ่ม Sign-In/Sign-Out, toggle เปิด/ปิด sync, sync status indicator, error banner แบบ dismissible); Domain = `src/hooks/useCloudSync.ts` (orchestrator — เป็น React hook เดียวที่ `App.tsx` เป็นเจ้าของ instance ตามรูปแบบเดียวกับ hook อื่นทั้งหมด); CloudSync (container "Cloud Synchronization Service") = `src/lib/cloudSync.ts` (`pullAndMerge`/`pushDiff` แบบ generic ต่อ collection บวก `pullProfile`/`pushProfile` แยกสำหรับ Profile ซึ่งเป็น single-record ไม่ใช่ array) ประกอบกับ `src/hooks/useAuth.ts` (Sign In/Sign Out ผ่าน `signInWithPopup` พร้อม fallback เป็น `signInWithRedirect` เมื่อ popup ถูกบล็อก — พบจริงตอนทดสอบตามที่บันทึกไว้ใน `backlog.md`/log Sprint 12); IdentityProvider = **Firebase Authentication** (เฉพาะ Google Sign-In provider เท่านั้นในรอบนี้ ไม่มี Email/Password); CloudStore = **Cloud Firestore** เก็บข้อมูลใต้ path `users/{uid}/{collection}/{id}` จำกัดสิทธิ์การเข้าถึงด้วย `firestore.rules` ที่ repo root (`request.auth.uid == uid`) — pull ครั้งเดียวต่อ entity ตอน sync เพิ่งเปิด+login สำเร็จ แล้ว push แบบ debounce 800ms ต่อ entity เมื่อ array เปลี่ยน (ค่า opt-in flag เก็บที่ LocalStorage key `my-today:sync-enabled` แยกต่างหากจาก Profile เอง เพราะเป็นการตัดสินใจต่ออุปกรณ์ ไม่ใช่ข้อมูลต่อบัญชี) — **หมายเหตุ:** `tech-stack.md` (เขียนไว้ 2026-08-23, ก่อน Sprint 12 จะเริ่ม) ยังไม่ครอบคลุมรายละเอียดนี้ รายละเอียดข้างต้นดึงมาจาก [[../../01-requirements/01-spec/20260829-014-my-today-sprint12-cloud-sync|Sprint 12 spec]] และ `backlog.md`/log การ implement จริงโดยตรงแทน — ควรรัน `tech-stack-advisor` อีกรอบเพื่อให้หมายเหตุนี้แม่นยำและมี source-of-truth เดียวในอนาคต (สอดคล้องกับหมายเหตุแบบเดียวกันที่ [[architecture|architecture.md]] หัวข้อ 2 และ [[api-spec|api-spec.md]] หัวข้อ 6 ระบุไว้แล้ว) — **นี่คือจุดแรกในเอกสารทั้งฉบับที่ message ในไดอะแกรมข้ามเครือข่ายจริง** ต่างจากที่หัวข้อถัดไปด้านล่าง ("เหตุผลที่ทุก message ในไดอะแกรมเป็น local call") เคยยืนยันไว้แบบเด็ดขาดว่าทุก message เป็น local call — ย่อหน้านั้นถูกแก้ไขให้มีเงื่อนไขแล้ว (ยกเว้นเฉพาะ §2.7 นี้)

### เหตุผลที่ทุก message ในไดอะแกรมอื่น (นอกเหนือจาก §2.7) เป็น local call

อ้างอิงตรงจาก [[tech-stack|tech-stack.md]] หัวข้อ 1 "Fixed Constraints" ("client-only, ไม่มี backend") และหัวข้อ 3 (Hosting = Vercel free tier, static SPA เท่านั้น) — ทุกลูกศรระหว่าง participant ในไดอะแกรมของหัวข้อ 1 และ 2.1-2.6 ข้างต้น implement เป็น TypeScript function call ภายใน process เดียวกัน (synchronous สำหรับ entity ที่ backed ด้วย LocalStorage, asynchronous ผ่าน Promise เฉพาะ entity File ที่ backed ด้วย IndexedDB) ไม่มีขั้นตอนใดข้าม network ออกจากเครื่องผู้ใช้เลย ทั้งหมด build ด้วย **Vite `^5.3.1`** และ deploy เป็น static SPA เดียวบน **Vercel free tier** — **ข้อยกเว้นเดียวในเอกสารทั้งฉบับคือ §2.7 (Enable Cloud Sync, Sprint 12)** ที่ message ไปยัง `IdentityProvider`/`CloudStore` implement เป็น network call จริงผ่าน Firebase SDK (ดูหัวข้อด้านบน) — สอดคล้องกับที่ [[architecture|architecture.md]] หัวข้อ 5 ระบุว่า Cloud Synchronization Service คือข้อยกเว้นแรกต่อหลักการ "no backend/local-first" ของทั้งระบบ และ [[api-spec|api-spec.md]] หัวข้อ 3 ระบุว่า "Push Diff to Cloud" คือ operation กลุ่มแรกที่ implement เป็น network call จริงเช่นกัน
...
- 20260830 (อัปเดตสะท้อน **Sprint 12 Cloud Sync build เสร็จสมบูรณ์แล้ว — ได้ sequence diagram จริงใหม่** และ **Sprint 13 Smart Capture จากรูปภาพ มีแค่ spec ยังไม่มีโค้ด — ยังคงเป็น known gap เท่านั้น**; ยืนยันแล้วจาก feature-journey-sync รอบล่าสุดว่า Sprint 12/13 ไม่อยู่ใน core persona journey ทั้งสอง จึง**ไม่มีการแก้ไข §1 เลย**) — **§2:** เพิ่ม **2.7 Enable Cloud Sync** เป็น sequence diagram เต็มรูปแบบใหม่ — participant ใหม่ `CloudSync` (Cloud Synchronization Service), `IdentityProvider` (External Identity Provider), `CloudStore` (Cloud Data Store) ทั้งสามชื่อดึงมาจาก container/external system ที่ [[architecture|architecture.md]] หัวข้อ 1/2 นิยามไว้แล้วเท่านั้น ไม่มีการเพิ่ม container ใหม่ในเอกสารนี้ — แสดง flow Sign In → Enable Cloud Sync (อ่าน local, pull+merge จาก cloud ด้วย last-write-wins เทียบ `updatedAt`, เขียน merge result กลับ local) พร้อมย่อหน้าร้อยแก้วอธิบาย Push Diff to Cloud (background debounce, ไม่เคย block การเขียน local ก่อนหน้า) ต่อท้ายตาม pattern เดียวกับ 2.6 — ระบุชัดว่านี่คือ**ไดอะแกรมแรกในเอกสารทั้งฉบับที่มี message ข้ามเครือข่ายจริง** — **§3:** เพิ่มโน้ต error/edge-case สำหรับ 2.7 (Sign In ล้มเหลว → dismissible error ไม่กระทบส่วนอื่น; Pull/Push ล้มเหลว → ข้อมูล local ไม่ถูกแตะต้อง ใช้งานต่อได้ปกติทันที) — **§4:** ตัด Sprint 12 ออกจาก known gaps (โปรโมทเข้า diagram จริงแล้ว) เพิ่ม bullet ใหม่ "Analyze Image for Event Capture" (Sprint 13) อธิบาย flow ที่วางแผนไว้แบบร้อยแก้ว (Sign In gate เดียวกับ 2.7 → Domain ส่งผ่าน Server-side Integration Proxy → External AI Analysis Service → เติมฟอร์ม Event ให้ผู้ใช้ตรวจสอบก่อนเสมอ) ไม่วาด diagram เพราะยังไม่มีโค้ดจริง ระบุชัดว่าจะเป็นไดอะแกรมที่ 2 ในเอกสารที่มี network call เมื่อ build จริง แยกจาก 2.7 โดยสิ้นเชิง — **§5:** เพิ่ม subsection ใหม่ "2.7 Enable Cloud Sync (Sprint 12)" อ้างอิงไฟล์จริง (`src/pages/ProfilePage.tsx`, `src/hooks/useCloudSync.ts`, `src/lib/cloudSync.ts`, `src/hooks/useAuth.ts`, Firebase Authentication/Cloud Firestore, `firestore.rules`) ดึงจาก spec Sprint 12/backlog.md โดยตรงเนื่องจาก `tech-stack.md` ยังไม่ครอบคลุม Sprint 12 (แนะนำรัน `tech-stack-advisor` อีกรอบ) — แก้หัวข้อปิดท้ายเดิม "เหตุผลที่ทุก message ในไดอะแกรมเป็น local call" ที่เคยยืนยันแบบเด็ดขาด ให้มีเงื่อนไข (เปลี่ยนชื่อหัวข้อเป็น "...ไดอะแกรมอื่น (นอกเหนือจาก §2.7)...") ระบุว่า §2.7 คือข้อยกเว้นเดียวที่เป็น network call จริง — **ไม่พบ conflict กับ [[architecture|architecture.md]]/[[api-spec|api-spec.md]]/[[database-schema|database-schema.md]] ที่อัปเดตวันเดียวกันแล้ว** (`tech-stack.md` ยังไม่ครอบคลุม Sprint 12 เช่นเดียวกับที่สองเอกสารนั้นระบุไว้แล้ว)
