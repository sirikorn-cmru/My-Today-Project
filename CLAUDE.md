...
`02-design/01-prototypes/` is active too, as of 2026-08-16 — it holds the User Journey docs (one per persona, see below). The rest of the later stages — `02-design/02-technical/`, `03-testing/`, `04-retrospectives/` — are still not active; don't create content there unless explicitly asked, since there's no technical design or test work to document until Sprint 8+ actually starts.
