...
- Entry: `index.html` → `src/main.tsx` (wraps `<App>` in `BrowserRouter`) → `src/App.tsx` (owns the single `useTasks()`, `useEvents()`, and `useFiles()` instances, passes data/CRUD callbacks down as props, renders `<Routes>` + `<NavBar>`)
- `src/pages/` — one component per route: `DashboardPage` (`/`), `TasksPage` (`/tasks`, search/filter/sort), `CalendarPage` (`/calendar`, Today/Week/Month views + event CRUD), `FilesPage` (`/files`, search + upload/preview/download/delete). All receive their data and CRUD callbacks as props from `App.tsx` — there's no context/store, since `App.tsx` is the single owner of state and every route is its direct child
- `src/components/` — presentational pieces used by the pages: `Header`, `SummaryCards`, `TodayTasks`, `TodaySchedule` (today's events, real data since Sprint 3), `Upcoming`, `NavBar`, `TaskCard` (shared row renderer for Today's Tasks and the Tasks page), `TaskFormModal` (Task create/edit — also renders the "Related Files" section + attach-existing-file picker when editing, using the optional `files`/`onLinkFile`/`onUnlinkFile` props), `EventFormModal` (Calendar event create/edit), `DayAgenda` (renders a merged, time-sorted list of events + task deadlines for one day — used by Calendar's Today/Week views), `FileFormModal` (file upload form, with an optional link-to-task checklist), `FileCard` (file row with inline Preview/Download/Delete)
- `src/hooks/useTasks.ts` / `src/hooks/useEvents.ts` / `src/hooks/useFiles.ts` — the only hooks that own Task / CalendarEvent / FileRecord state respectively. Tasks and Events persist to LocalStorage (seeding `src/data/seedTasks.ts` / `src/data/seedEvents.ts` on first run); Files persist to IndexedDB via `src/lib/fileDb.ts` (seeding `src/data/seedFiles.ts` on first run) since file blobs don't fit LocalStorage's string-only, ~5-10MB quota — `useFiles` exposes a `loaded` flag pages must check before rendering, since IndexedDB reads are async unlike the other two hooks' synchronous LocalStorage reads. Only `App.tsx` calls these hooks — don't call them again elsewhere, it would create a second out-of-sync copy of the data
- `src/lib/storage.ts` — generic typed LocalStorage read/write JSON helpers (used by Tasks/Events, not Files)
