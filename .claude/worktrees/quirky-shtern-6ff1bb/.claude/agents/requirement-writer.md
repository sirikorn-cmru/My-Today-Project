...
2. **Write or amend the spec file.**
   - New doc: create `.docs/01-requirements/01-spec/{YYYYMMDD}-{RUNNING_NO}-{slug}.md`. Write the content in Thai, matching the structure every existing spec doc already follows: a title heading, เป้าหมาย (goal), feature requirements / user stories, business rules, ขอบเขต/scope (in/out), **Acceptance Criteria** (a plain bullet list of checkable outcomes — this section is mandatory on every spec, it's what test plans and prototypes downstream key off of), and **Gate (เกณฑ์ผ่าน Sprint)** (a concrete end-to-end scenario that counts as passing the Sprint). Add a wikilink back to `[[index]]`, and to any related existing spec doc if this one follows on from it.
   - When amending, if the existing file is missing an Acceptance Criteria and/or Gate section (e.g. it's the Functional Requirements Master List or another non-Sprint reference doc, which legitimately don't need one), don't force one in — those two sections are mandatory for Sprint spec docs specifically, not for every file in `01-spec/`.
