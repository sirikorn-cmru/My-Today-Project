...
ตั้งแต่มี [[../../.docs/02-design/02-technical/tech-stack|tech-stack.md]] (เอกสารตัดสินใจ stack จริง ผูกกับเทคโนโลยีโดยเจตนา) แล้ว — เชิงอรรถ "หมายเหตุการ implement ปัจจุบัน" ท้ายเอกสารนี้ควร**ลงรายละเอียดให้แม่นยำและเจาะจงยิ่งขึ้น** โดยอ้างอิงชื่อ/เวอร์ชันเทคโนโลยีจริงจาก tech-stack.md ตรงๆ (เช่น ระบุ "React 18.3 + React Router 7" แทนคำกว้างๆ ว่า "framework ฝั่ง UI") แทนที่จะเดาหรือเขียนกว้างๆ เอง — หลักการ "ห้ามผูกกับ stack ในเนื้อหาหลัก" ยังคงเดิมทุกประการ เปลี่ยนแค่ความละเอียด/ความแม่นยำของเชิงอรรถท้ายเอกสารเท่านั้น
...
- `.docs/02-design/02-technical/tech-stack.md` (ถ้ามีอยู่แล้ว) — แหล่งอ้างอิงหลักสำหรับรายละเอียดที่จะใส่ในเชิงอรรถ "หมายเหตุการ implement ปัจจุบัน" ของแต่ละ Container (ชื่อ/เวอร์ชันเทคโนโลยีที่แม่นยำต่อ layer เช่น Presentation ใช้อะไร, Domain Logic ใช้อะไร, Persistence แต่ละชนิดใช้อะไร) — ถ้ายังไม่มีไฟล์นี้ ใช้แนวทางเดิม (เดาจาก `package.json`/config อย่างระมัดระวัง) และแจ้ง user ว่ายังไม่มี tech-stack.md ให้พิจารณารัน `tech-stack-advisor` ก่อนถ้าต้องการหมายเหตุที่แม่นยำที่สุด
...
- บอก subagent ให้อ่าน `.docs/02-design/02-technical/tech-stack.md` เอง (ถ้ามี) เพื่อดึงรายละเอียดเทคโนโลยีที่แม่นยำมาใส่ในเชิงอรรถต่อ Container — ไม่ต้องสรุปเนื้อหาให้ล่วงหน้า เพราะ subagent อ่านไฟล์เองได้
