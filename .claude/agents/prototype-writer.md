...
- The source documents to draw screen content from: the relevant spec file(s) in `.docs/01-requirements/01-spec/`, the matching `backlog.md` entries, and — for scope that's already covered by them — `.docs/01-requirements/feature-list.md` and the relevant `.docs/02-design/01-prototypes/user-journey-{persona}.md` file(s). If the scope includes a Sprint that predates those two docs' latest run (e.g. a spec just created via `requirement-intake` that `feature-journey-sync` hasn't processed yet), the caller will instead hand you the spec's own "Feature Requirements"/Business Rules/Acceptance Criteria sections to derive the equivalent content from directly — the caller will tell you explicitly which mode applies.
...
2. **Read the source documents** given to you (spec file(s), backlog entries, and `feature-list.md`/`user-journey-{persona}.md` or the derived spec sections) to understand what screens/flows are actually needed: what fields, actions, states (empty/loading/error where relevant), and navigation between screens the spec describes.
...
6. **Write `index.md`** in the version folder (Thai, matching the vault's existing style) summarizing: which scope/Sprint this prototype version covers, which screens are included and what each one shows, wikilinks back to the source spec doc(s) and backlog entries used, and — if `feature-list.md`/`user-journey-{persona}.md` content had to be derived from a spec directly instead (because that Sprint isn't in those docs yet) — a note saying so and from which section it was derived. Link back to `[[../index|01-prototypes]]`.
