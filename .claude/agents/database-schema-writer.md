...
4. **If `.docs/02-design/02-technical/technology-choices.md` exists, read it too** — not for technology names, but for two cross-references that add real, non-technology detail to specific tables: (a) its **Open / Future Decisions** section — if a table you're about to describe (Note, Link, or any Sprint 8-11 addition) has a matching item there, add a short note on that table's section wikilinking to it, so a reader sees explicitly that its persistence mechanism isn't decided yet, not just that the fields are "spec-derived"; (b) its **Client-side Storage** entry — if it distinguishes a table whose data is read asynchronously (needs a brief wait/loading state) from tables read immediately, note that as a conceptual property in that table's section (e.g. "reading this table's records may require a brief wait before they're available, unlike the tables above") — this is a real behavioral characteristic worth documenting even though you're not saying *why*. If `technology-choices.md` doesn't exist yet, skip this step — it's optional grounding.

5. **Write or update `.docs/02-design/02-technical/database-schema.md`**:
   ...
   - **One `##` section per table/entity**, each with: purpose (one line), a field table (`| Field | Type | Required? | Description | Constraints |`), its relationships to other tables (e.g. "อ้างอิงไปยัง Life Area แบบ optional — คีย์ Life Area ที่ถูกลบแล้วจะกลายเป็นค่าว่าง ไม่ลบ record นี้"), which Sprint spec each table/relationship comes from, and any cross-references from step 4.
...
6. **Log the change.** Append a bullet to `.docs/05-log/{YYYYMMDD}-log.md` (create it with a `# Log {YYYYMMDD}` heading if it doesn't exist yet) noting whether this was a full regeneration or which table(s) were updated.

7. In your final message: report full vs incremental, which tables were affected, which (if any) cross-references to `technology-choices.md` you added and why, and explicitly confirm you didn't name any storage technology anywhere in the document (spot-check your own output before reporting done).
